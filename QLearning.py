...
class QAgent(object):
    ...
    def predict_batch(self, states):#生成动作选择
        epsilon = self.epsilons[min(self.total_t, self.epsilon_decay_steps-1)]
        A = np.ones((len(states), self.action_num), dtype=float) * epsilon / self.action_num #对每个中心节点平均生成动作选择概率
        q_values = self.q_estimator.predict_nograd(states)
        best_action = np.argmax(q_values.cpu(), axis=1)#使用Q函数生成最优动作
        for node_index, a in enumerate(best_action):
            A[node_index][a] += (1.0 - epsilon)#对最优动作位置的值概率增加至1
            A[node_index] = A[node_index] / sum(A[node_index])#概率归一化
        return A
    ...
    def train(self):
        ...
        # depth-RL
        q_values_next = self.q_estimator.predict_nograd(next_state_batch)#生成下一步Q值
        best_actions = np.argmax(q_values_next.cpu(), axis=1)#决策选择最大概率动作
        ...
        if self.train_t % self.update_target_estimator_every == 0:
            self.target_estimator = deepcopy(self.q_estimator) #更新目标函数
    ...
    def predict_nograd(self, states):
        ...
        with torch.no_grad():
            ...
            for state in states:
                ...
                q_as = self.qnet(torch.cat((node_embedding, neighbor_embedding), 1))#cat 函数是间输入的两个矩阵进行拼接。
                Q.append(q_as)
        return torch.cat(Q)
