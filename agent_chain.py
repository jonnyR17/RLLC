...
class AgentChain(object):
    ...
    def predict(self, graph_node_embedding, graph, batch):
        ...
        #     # x = graph.x[neighbors]
        for community in self.communites:
            neighbors = torch.as_tensor(list(community), dtype=torch.long,
                                        device=graph_node_embedding.device)

            # 过滤非法索引
            neighbors = neighbors[(neighbors >= 0) & (neighbors < graph.num_nodes)]
            neighbors = neighbors.unique()

            if neighbors.numel() == 1:
                node = neighbors.item()
                src, dst = graph.edge_index
                src = src.to(graph_node_embedding.device).long()
                dst = dst.to(graph_node_embedding.device).long()

                one_hop_neighbors = torch.cat([dst[src == node], src[dst == node]]).unique()
                neighbors = torch.cat([neighbors, one_hop_neighbors]).unique()
            ...
            edge_index,edge_attr = subgraph(neighbors, graph.edge_index, edge_attr=graph.edge_attr,relabel_nodes=True)
